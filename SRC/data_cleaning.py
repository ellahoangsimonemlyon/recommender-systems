...
print(f"\n\nProduct_info missing values: {product_info.isnull().sum().to_frame(name='missing_count').assign(missing_percent=lambda x: 100 * x['missing_count'] / len(product_info)).sort_values(by='missing_count', ascending=False)}")

# Clean missing values
cols_to_drop = ['sale_price_usd', 'value_price_usd', 'variation_desc', 'child_max_price', 'child_min_price', 'size', 'variation_value', 'variation_type', 'child_count']
product_info_cleaned = clean_data(product_info, columns_to_drop=cols_to_drop, do_imputation=True)  # 0.17 chosen based on the column type and the percentage of missing values

## Change the data type of reviews column to int.
product_info_cleaned['reviews'] = product_info_cleaned['reviews'].astype(int)
product_info_cleaned['loves_count'] = product_info_cleaned['loves_count'].astype(int)
...
print(f"\n\nReviews missing values: {reviews_df.isnull().sum().to_frame(name='missing_count').assign(missing_percent=lambda x: 100 * x['missing_count'] / len(reviews_df)).sort_values(by='missing_count', ascending=False)}")

cols_to_drop = ['helpfulness', 'review_title', 'Unnamed: 0']
reviews_df_cleaned = clean_data(reviews_df, columns_to_drop=cols_to_drop)  # 0.25 based on the number of null values present in the dataset

## Change data time data types
reviews_df_cleaned['submission_time'] = pd.to_datetime(reviews_df_cleaned['submission_time'], errors='coerce')

## change column values to lower case
reviews_df_cleaned['skin_tone'] = reviews_df_cleaned['skin_tone'].str.lower().str.strip()
reviews_df_cleaned['brand_name'] = reviews_df_cleaned['brand_name'].str.lower().str.strip()
reviews_df_cleaned['product_name'] = reviews_df_cleaned['product_name'].str.lower().str.strip()
