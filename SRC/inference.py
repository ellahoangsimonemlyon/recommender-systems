...
class SephoraInferenceSystem:
    ...
    def _get_existing_user_recommendations(
        ...
        user_rated_products = set(
            self.processed_data["reviews_df"][
        ...
        if self.cf_model is not None:
            try:
                cf_recs, cf_scores = self.cf_model.recommend(
                    user_idx,
                    self.mappings["user_item_matrix"][user_idx],
    ...
    def _get_content_based_recommendations(
        ...
        if (
            self.cb_model["content_similarity_matrix"] is None
            or not user_rated_products
            or not self.cb_model["content_product_to_idx"]
        ):
            return []
        ...
        try:
            ...
            similarity_scores = self.cb_model["content_similarity_matrix"][
                valid_rated_indices
            ].mean(axis=0)
    ...
    def _get_popular_products(self, n_products):
        ...
        try:
            ...
            return list(
                self.processed_data["products_df"]["product_id"].head(n_products)
            )
        ...
        for col in category_columns:
            if col in self.processed_data["products_df"].columns:
                categories[col] = sorted(
                    self.processed_data["products_df"][col].dropna().unique()
                )
            else:
                categories[col] = []
    ...
    def get_user_stats(self):
        ...
        stats = {
            ...
            "model_timestamp": (
                self.metadata.get("timestamp", "Unknown")
                if self.metadata
                else "Unknown"
            ),
        }
...
def test_inference_system():
    ...
    if users:
        test_user = users[0]
        recs = inference_system.get_user_recommendations(
            user_id=test_user, n_recommendations=5
        )
    ...
    recs = inference_system.get_user_recommendations(
        user_profile=test_profile, n_recommendations=5
    )
