...
class RecommendationEvaluator:
            ...
            if self.inference_system.cf_model is not None:
                ...
                if user_item_matrix is not None:
                    try:
                        ...
                        print(f"   Train matrix: {self.train_matrix.shape}, nnz: {self.train_matrix.nnz}")
                        if self.test_matrix is not None:
                            print(f"   Test matrix: {self.test_matrix.shape}, nnz: {self.test_matrix.nnz}")
                    except Exception as e:
                        print(f"⚠ Could not create train/test split: {e}")
                        self.train_matrix = user_item_matrix
                        self.test_matrix = None
            else:
                print("⚠ No collaborative filtering model available for advanced evaluation")
        except Exception as e:
            print(f"⚠ Error loading trained model: {e}")
    
    def evaluate_rmse(self):
        """Evaluate RMSE for rating prediction on existing users"""
        print("Evaluating RMSE (Rating Prediction)...")
        ...
        if self.trained_model is None or self.test_matrix is None:
            ...
            self.results['rmse'] = {'rmse': None, 'mae': None, 'reason': 'No test data available'}
            return self.results['rmse']
        
        try:
            # Get users with test data
            users_with_test_data = np.where(np.diff(self.test_matrix.indptr) > 0)[0]
            sample_users = users_with_test_data[:min(500, len(users_with_test_data))]  # Sample for efficiency
            ...
            if len(predicted_ratings) > 0:
                ...
                print(f"✓ Evaluated on {len(predicted_ratings):,} predictions from {len(sample_users)} users")
                
            else:
                print("⚠ No predictions could be made")
                self.results['rmse'] = {'rmse': None, 'mae': None, 'reason': 'No predictions possible'}
    ...
    def ranking_metrics_at_k(self, K=10):
        ...
        if self.trained_model is None or self.train_matrix is None or self.test_matrix is None:
            print("⚠ Cannot evaluate ranking metrics: No trained model or test data available")
            return {'precision': None, 'map': None, 'ndcg': None, 'reason': 'No test data available'}
        ...
        try:
            ...
            sample_users = users_with_test_data[:min(200, len(users_with_test_data))]  # Sample for efficiency
            ...
            while start_idx < len(sample_users):
                ...
                try:
                    recommended_items, scores = self.trained_model.recommend(
                        batch_users, train_user_items[batch_users], N=K, filter_already_liked_items=True
                    )
                except Exception as e:
                ...
                for user_idx, user_id in enumerate(batch_users):
                    ...
                    user_recommendations = recommended_items[user_idx] if len(recommended_items.shape) > 1 else recommended_items
            ...
            precision_at_k = relevant / total_precision_div if total_precision_div > 0 else 0
    ...
    def create_evaluation_report(self, save_path="evaluation_report.html"):
        ...
        if 'rmse' in self.results and self.results['rmse'].get('rmse') is not None:
            ...
            rmse_class = 'good' if rmse_data['rmse'] < 1.0 else 'warning' if rmse_data['rmse'] < 1.5 else 'poor'
            html_content += f"""
        ...
        # Add MAP@10 metrics
        if 'map_at_10' in self.results and self.results['map_at_10'].get('map') is not None:
            map_data = self.results['map_at_10']
            map_class = 'good' if map_data['map'] > 0.1 else 'warning' if map_data['map'] > 0.05 else 'poor'
            html_content += f"""
        ...
        if self.results.get('rmse', {}).get('rmse') and self.results['rmse']['rmse'] > 1.5:
            html_content += "<li>⚠️ High RMSE - consider tuning model hyperparameters or adding more features</li>"
        elif self.results.get('rmse', {}).get('rmse') and self.results['rmse']['rmse'] < 1.0:
            html_content += "<li>✅ Good RMSE performance - model predicts ratings accurately</li>"
        
        if self.results.get('map_at_10', {}).get('map') and self.results['map_at_10']['map'] > 0.1:
            html_content += "<li>✅ Good MAP@10 performance - model ranks relevant items well</li>"
        elif self.results.get('map_at_10', {}).get('map') and self.results['map_at_10']['map'] < 0.05:
            html_content += "<li>⚠️ Low MAP@10 - improve ranking quality with better features or algorithms</li>"
    ...
    def print_summary(self):
        ...
        if 'rmse' in self.results and self.results['rmse'].get('rmse') is not None:
            ...
            if rmse_data['rmse'] < 1.0:
                ...
                print("    ⚠️ Poor rating prediction - consider model tuning")
        else:
        ...
        if 'map_at_10' in self.results and self.results['map_at_10'].get('map') is not None:
            ...
            if map_data['map'] > 0.1:
                ...
                print("    ⚠️ Poor ranking performance - improve recommendation quality")
        else:
        ...
        if 'popularity_bias' in self.results:
            print(f"  • Popularity Bias: {self.results['popularity_bias']['bias_score']:.3f}")
        
        if 'cold_start' in self.results:
            print(f"  • Cold Start Success: {self.results['cold_start']['success_rate']:.2%}")
        ...
        if self.results.get('map_at_10', {}).get('map') and self.results['map_at_10']['map'] < 0.05:
            print("  • Improve content-based features or add more user interaction data")
