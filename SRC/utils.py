...
class SephoraRecommendationSystem:
    ...
    def _prepare_data(self):
        ...
        try:
            ...
            if missing_review_cols:
                print(
                    f"ERROR: Missing required columns in reviews_df: {missing_review_cols}"
                )
                return False
            if missing_product_cols:
                print(
                    f"ERROR: Missing required columns in products_df: {missing_product_cols}"
                )
                return False
    ...
    def _clean_data(self):
        ...
        self.products_df["price_usd"] = self.products_df["price_usd"].fillna(
            self.products_df["price_usd"].median()
        )

        # Convert string representations to actual strings for TF-IDF
        self.products_df["ingredients_text"] = self.products_df["ingredients"].astype(
            str
        )
    ...
    def _create_user_item_matrix(self):
        ...
        try:
            ...
            self.product_to_idx = {
                product: idx for idx, product in enumerate(unique_products)
            }
            self.idx_to_product = {
                idx: product for product, idx in self.product_to_idx.items()
            }
            ...
            self.user_item_matrix = csr_matrix(
                (data, (rows, cols)), shape=(len(unique_users), len(unique_products))
            )
    ...
    def _create_content_features(self):
        ...
        try:
            ...
            products_filtered = products_filtered.sort_values("product_id").reset_index(
                drop=True
            )
            ...
            self.content_product_to_idx = {
                product_id: idx
                for idx, product_id in enumerate(products_filtered["product_id"])
            }
            self.content_idx_to_product = {
                idx: product_id
                for product_id, idx in self.content_product_to_idx.items()
            }
    ...
    def _create_user_profiles(self):
        ...
        try:
            ...
            available_features = [
                f for f in profile_features if f in self.reviews_df.columns
            ]
            ...
            for features, group in self.reviews_df.groupby(
                available_features, dropna=False
            ):
                if len(group) >= 5:  # Only consider profiles with enough data
                    # Get popular products for this profile
                    popular_products = (
                        group.groupby("product_id")["rating"]
                        .agg(["mean", "count"])
                        .reset_index()
                    )
                    ...
                    profile_key = (
                        tuple(features) if isinstance(features, tuple) else (features,)
                    )
                    self.user_profiles[profile_key] = (
                        popular_products["product_id"].head(20).tolist()
                    )
    ...
    def train_models(self, test_size=0.2):
        ...
        try:
            if self.user_item_matrix is None:
                raise ValueError(
                    "User-item matrix is None. Data preparation may have failed."
                )
            ...
            try:
                train_matrix, test_matrix = train_test_split(
                    self.user_item_matrix, train_percentage=1 - test_size
                )
    ...
    def _get_existing_user_recommendations(
        ...
        user_rated_products = set(
            self.reviews_df[self.reviews_df["author_id"] == user_id][
                "product_id"
            ].values
        )
        ...
        if self.user_item_model is not None:
            try:
                ...
                cf_recs = (
                    cf_recs.tolist() if hasattr(cf_recs, "tolist") else list(cf_recs)
                )
                cf_scores = (
                    cf_scores.tolist()
                    if hasattr(cf_scores, "tolist")
                    else list(cf_scores)
                )

                # Convert indices to product IDs
                cf_product_ids = [
                    self.idx_to_product[idx]
                    for idx in cf_recs
        ...
        cb_product_ids = self._get_content_based_recommendations(
            user_rated_products, n_recommendations * 2
        )
        ...
        for product_id in cf_product_ids:
            if (
                product_id not in seen_products
                and len(combined_recs) < n_recommendations * 2
            ):
        ...
        if not filtered_recs and any(
            [primary_category, secondary_category, tertiary_category]
        ):
            print(
                f"No recommendations found for user {user_id} in specified categories. Using popular products in category."
            )
    ...
    def _get_content_based_recommendations(
        self, user_rated_products, n_recommendations
    ):
        """Get content-based recommendations using product similarity"""
        if (
            self.content_similarity_matrix is None
            or not user_rated_products
            or not self.content_product_to_idx
        ):
            return []
        ...
        try:
            ...
            if not valid_rated_indices:
                print(
                    f"Warning: No valid content indices found for user's rated products"
                )
                return []

            # Calculate average similarity scores
            similarity_scores = self.content_similarity_matrix[
                valid_rated_indices
            ].mean(axis=0)
            ...
            if self.content_similarity_matrix is not None:
                ...
                if rated_indices:
                    ...
                    print(
                        f"Max rated index: {max(rated_indices) if rated_indices else 'None'}"
                    )
            return []
        ...
        if user_profile and self.user_profiles:
            ...
            if profile_key in self.user_profiles:
                ...
                for stored_profile, products in self.user_profiles.items():
                    score = sum(
                        1
                        for i, val in enumerate(profile_key)
                ...
                product_recs = (
                    best_match
                    if best_match
                    else self._get_popular_products(n_recommendations * 2)
                )
        else:
    ...
    def _get_popular_products(self, n_products):
        ...
        try:
            ...
            popular["popularity_score"] = popular["avg_rating"] * np.log1p(
    ...
    def _get_popular_products_by_category(
        self, primary_category, secondary_category, tertiary_category, n_products
    ):
        ...
        try:
            ...
            result = self._get_category_products_with_ratings(
                primary_category, secondary_category, tertiary_category, n_products
            )
            if result:
            ...
            for (
                ...
                strategy_secondary,
                strategy_tertiary,
            ) in fallback_strategies:
                if any(
                    [strategy_primary, strategy_secondary, strategy_tertiary]
                ):  # At least one filter
                    result = self._get_category_products_with_ratings(
                        strategy_primary,
                        strategy_secondary,
                    ...
                    if result:
                        print(
                            f"Fallback: Found {len(result)} products using relaxed category filters"
                        )
                        return result

            # Strategy 3: Use products from category even without ratings (content-based)
            result = self._get_unrated_category_products(
                primary_category, secondary_category, tertiary_category, n_products
            )
            if result:
                print(
                    f"Fallback: Found {len(result)} unrated products in category using content similarity"
                )
                return result

            # Strategy 4: Find similar categories and use their products
            result = self._get_similar_category_products(
                primary_category, secondary_category, tertiary_category, n_products
            )
            if result:
    ...
    def _get_category_products_with_ratings(
        self, primary_category, secondary_category, tertiary_category, n_products
    ):
        ...
        try:
                ...
                try:
                    ...
                    for product_id in valid_product_ids:
                        if product_id in self.product_to_idx:
                            ...
                            item_ratings = self.reviews_df[
                                self.reviews_df["product_id"] == product_id
                            ]["rating"]
                            avg_rating = (
                                item_ratings.mean() if len(item_ratings) > 0 else 3.0
                            )

                            # Combine ALS factors with interactions and ratings
                            if item_idx < len(item_factors):
                                factor_magnitude = np.linalg.norm(
                                    item_factors[item_idx]
                                )
                                popularity_score = (
                                    factor_magnitude * 0.3
                                    + float(item_interactions) * 0.4
                                    + avg_rating * 0.3
                                )
                            else:
                                popularity_score = (
                                    float(item_interactions) * 0.7 + avg_rating * 0.3
                                )

                            popularity_scores[product_id] = popularity_score

                    sorted_products = sorted(
                        popularity_scores.items(), key=lambda x: x[1], reverse=True
                    )
                    return [
                        product_id for product_id, score in sorted_products[:n_products]
                    ]
            ...
            category_reviews = self.reviews_df[
                self.reviews_df["product_id"].isin(valid_product_ids)
            ]
            ...
            popular = (
                category_reviews.groupby("product_id")
                .agg({"rating": ["mean", "count"]})
                .reset_index()
            )
            popular.columns = ["product_id", "avg_rating", "review_count"]
    ...
    def _get_unrated_category_products(
        self, primary_category, secondary_category, tertiary_category, n_products
    ):
        ...
        try:
            ...
            if "price_usd" in filtered_products_df.columns:
                filtered_products_df["completeness_score"] += (
                    filtered_products_df["price_usd"].notna().astype(int) * 2
                )
            if "brand_name" in filtered_products_df.columns:
                filtered_products_df["completeness_score"] += (
                    filtered_products_df["brand_name"].notna().astype(int) * 1
                )
            if "ingredients" in filtered_products_df.columns:
                filtered_products_df["completeness_score"] += (
                    filtered_products_df["ingredients"].notna()
                    & (filtered_products_df["ingredients"] != "")
                ).astype(int) * 3
            if "highlights" in filtered_products_df.columns:
                filtered_products_df["completeness_score"] += (
                    filtered_products_df["highlights"].notna()
                    & (filtered_products_df["highlights"] != "")
                ).astype(int) * 2

            # Sort by completeness score and take top products
            filtered_products_df = filtered_products_df.sort_values(
                "completeness_score", ascending=False
            )
        ...
        try:
            ...
            if primary_category:
                ...
                for cat, similar in category_similarities.items():
                    if (
                        primary_category.lower() in cat.lower()
                        or cat.lower() in primary_category.lower()
                    ):
            ...
            for similar_cat in similar_categories:
                result = self._get_category_products_with_ratings(
                    similar_cat, None, None, n_products
                )
                if result:
                    return result
    ...
    def _apply_category_filters(
        self, product_ids, primary_category, secondary_category, tertiary_category
    ):
        ...
        for product_id in product_ids:
            ...
            if (
                primary_category
                and product_info.get("primary_category") != primary_category
            ):
                continue
            if (
                secondary_category
                and product_info.get("secondary_category") != secondary_category
            ):
                continue
            if (
                tertiary_category
                and product_info.get("tertiary_category") != tertiary_category
            ):
                continue
                ...
                recommendations.append(
                    {
                        ...
                        "primary_category": product_info.get(
                            "primary_category", "Unknown"
                        ),
                        "secondary_category": product_info.get(
                            "secondary_category", "Unknown"
                        ),
                        "tertiary_category": product_info.get(
    ...
    def get_user_stats(self):
        ...
        stats = {
            ...
            "total_interactions": (
                self.user_item_matrix.nnz if self.user_item_matrix is not None else 0
            ),
            "sparsity": (
                (
                    1
                    - self.user_item_matrix.nnz
            ...
            "avg_ratings_per_product": self.reviews_df.groupby("product_id")
            .size()
            .mean(),
        }
        return stats
